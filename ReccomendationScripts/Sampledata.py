...
for x in range(int(ttlfiles)):
    ...
    thirties = []
    thirty_ones = []
    if ((randmonth == 2) and (randdate == 29) and (randyear%4 != 0)) or ((randmonth == 2) and (randdate > 29)):  #lask condition kinda iffy.
        continue
    """
    ...
    #prevents duplication likkle bit
    mingoal = min(goals)
    """ 
    if (mingoal in goals) and (len(goals) > 1):
